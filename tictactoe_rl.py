...
# Set all diagonal flags to True (In paper its not clear if they are counted or not)
USE_DIAGONALS_FOR_OPEN_COUNTS = True      # affects x1,x2
USE_DIAGONALS_FOR_TWOS        = True      # affects x3,x4
USE_DIAGONALS_FOR_THREES      = True      # affects x5,x6
...
def features(b: Board, me: str) -> List[float]:
    ...
    # x1, x2: counts of own/opponent marks in "open" lines (no enemy there)
    for a, b1, c in LINES_OPEN:
        line = [b[a], b[b1], b[c]]
        if you not in line: x1 += line.count(me)
        if me  not in line: x2 += line.count(you)

    # x3, x4: two-in-a-row + one empty on chosen lines
    ...
    # x5, x6: completed three-in-a-row on chosen lines
...
class LinearValue:
    ...
    def update_lms(self, b: Board, me: str, target: float, xs: List[float]):
        """
        MODIFIED: Accepts pre-calculated features 'xs' as an argument.
        """
        self._finite()
        # clip target
...
class PerformanceSystem:
    ...
    def choose_best(self, b: Board, player: str) -> int:
        ...
        # Original Greedy Policy (if not exploring)
        opp=O if player==X else X
        best,score=None,None
        moves=legal_moves(b)
        random.shuffle(moves)  # random tie-break
        for mv in moves:
            nb=apply(b,mv,player)
            s = self.v.predict(nb, player)  # maximize own value
...
class Critic:
    ...
    def build_examples(self, trace:List[Tuple[Board,str]], res:str, v:LinearValue):
        """
        MODIFIED: Now pre-calculates features and passes them along.
        """
        if not trace: return []
        
        # 1. Calculate the utility for EACH player's perspective
        util_X = self.util(res, 'X') # Will be 100, 0, or -100
        util_O = self.util(res, 'O') # Will be 100, 0, or -100
        
        ex=[]
        for s_t,p_t in trace:
            feats_t = features(s_t, p_t) 
            
            # Assign the correct target utility based on whose turn it was
            if p_t == 'X':
                ex.append((s_t, p_t, util_X, feats_t)) 
            else: # p_t == 'O'
                ex.append((s_t, p_t, util_O, feats_t))

        s_final, p_winner = trace[-1]
        
        if res != 'D': # If it's not a draw
            p_loser = 'O' if p_winner == 'X' else 'X'
            util_loser = util_X if p_loser == 'X' else util_O 
            
            # Pre-calculate features for the loser's final state
            feats_loser = features(s_final, p_loser)
            ex.append((s_final, p_loser, util_loser, feats_loser))
...
class Generalizer:
    ...
    def fit(self,examples):
        for b,p,t,feats in examples: 
            self.v.update_lms(b, p, t, feats) 

class ExperimentGenerator:
    def __init__(self, perf:PerformanceSystem): 
        self.perf = perf

    def self_play(self, start: str) -> Tuple[List[Tuple[Board, str]], str]:
        b = [E]*9
        p = start
        trace: List[Tuple[Board, str]] = []
...
# ------------ Agent wrapper ------------
class Agent:
    def __init__(self, lr=0.4, preset=None):
        base=0.5
    ...
    # ---- evaluation with fixed weights (paper-style self-play) ----
    def evaluate_self_play(self, n:int, snap_every:int=0, log_cb=None):
        original_epsilon = self.perf.epsilon
        self.perf.epsilon = 0.0  # Turn off exploration for evaluation
        ...
        # restore running totals (evaluation shouldn't contaminate training counters)
...
class App:
    def __init__(self, root):
        ...
        self.agent=Agent()
        self.human_side = tk.StringVar(value="O")  # default: agent starts
        self.board=[E]*9
        self._build_ui()
        self._new_game()
    ...
    def apply_preset(self):
        ...
        self.agent=Agent(preset=PRESET_WEIGHTS[k])   # fresh agent with given weights
    ...
    def plot_weights(self):
        if not self.agent.hist_w:
            # show current weights if no history
            plt.figure()
